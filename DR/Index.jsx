...
export const NavBar = () => {
  ...
  const [scrolling, setScrolling] = useState(false); // Flag to indicate scrolling state
  const [navigationOccurred, setNavigationOccurred] = useState(false); // Flag to indicate if navigation has occurred
  const prevScrollX = useRef(window.scrollX);
  ...
  //       !navigationOccurred // Adjust this value as needed
  //     ) {
  //       setNavigationOccurred(true); // Set flag to indicate navigation has occurred
  //       navigateToPreviousPage();
  //     }
  //   };

  //   document.addEventListener("wheel", handleScroll);
  //   document.addEventListener("touchmove", handleScroll); // Add touch event listener

  //   return () => {
  //     document.removeEventListener("wheel", handleScroll);
  //     document.removeEventListener("touchmove", handleScroll); // Remove touch event listener
  //   };
  // }, [navigate, navigationOccurred]); // Listen to changes in navigation state and navigation occurred flag
  ...
  const handleLinkClick = (page) => {
    ...
    setNavigationOccurred(false); // Reset navigation occurred flag when link is clicked
  };
  const LightFunction = () => {
    if (Light === false) {
      setLight(true);
  ...
  useEffect(() => {
    ...
    if (Light) {
      ...
      document.body.style.fill = "black";
      ...
      // Reset body background gradient
    }
  }, [Light]);
